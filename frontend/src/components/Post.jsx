...
const Post = ({ post }) => {
  ...
  const isInitiallyBookmarked = user?.bookmarks?.some(b => b === post._id);
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://insta-clone-backend-eeh5.onrender.com/api/v1/post/${post._id}/${action}`, { withCredentials: true });
      if (res.data.success) {
        const updatedLikes = liked ? postLike - 1 : postLike + 1;
        setPostLike(updatedLikes);
        setLiked(!liked);

        //updated post logic here
  ...
  const commentHandler = async () => {
    try {
      const res = await axios.post(`https://insta-clone-backend-eeh5.onrender.com/api/v1/post/${post._id}/comment`, { text }, {
        headers: {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(`https://insta-clone-backend-eeh5.onrender.com/api/v1/post/delete/${post?._id}`, { withCredentials: true })
      if (res.data.success) {
    ...
    <div className='my-8 w-full max-w-sm mx-auto'>
      ...
      <div className='flex item-center justify-between my-2'>
        ...
        <Bookmark onClick={bookmarkHandler}
          className={`cursor-pointer hover:text-gray-600 ${isBookmarked ? 'text-black fill-black' : ''}`}
        />
      </div>
